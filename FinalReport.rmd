---
title: "GSS challange Less Data"
author: "Daisy Gallegos"
date: "Summer 2024"
output: html_document
---







```{r include=FALSE,echo=FALSE}
require(tidyverse)
require(tigerstats)
require(knitr)
require(foreign)
GSS <-read.dta(file="GSS2016.dta")
GSS <- droplevels(GSS)
mydata <- select(GSS,sex,race,cappun,degree,relig,gunlaw,tvhours,owngun,age,wordsum)
mydata <- na.omit(mydata)
```

# Introduction

<!--In this section you explain what you are trying to show.  Where did the data come from?  What is the research or other question you are trying to answer?.  If you skimp in this section expect to be marked down in your grade. Paint a picture of the project a couple of paragraphs is expected!-->

Is attitude toward capitol punishment (cappun) independent of race (race)?
If not, then whose view differs from whom??

# Methods

<!--Decide on your methods:  use "variable analysis" or other appropriate descriptors.  Make sure to choose at least one graphical method and at least one numerical method. Discuss what data manipulations may be needed for example sub-setting or other binning procedures and what sort of graphical displays and inferential tools will be used!-->
  
In this area we will be duplicating what we did in the master branch except we have a lot more variables to deal with and the results are going to cause us to have a lot less data of the items of interest. You will have a lot less data points per time point because it's requiring that each variable has a value and the more variables you have, the less of them there will go to be where each and every one of them has a value. This time we got less data points because we have more data in the requirement by reading in all those various measurement results in us getting a lot less data points in the end.
  
# Results

<!--Divide this section into two sub-sections:  One for your descriptive  results and one for your inferential results.!-->

## Descriptive Results

### Graphical Descriptive Results

<!--Graphical results here.  Make sure to show your code.  Provide appropriate labels for axes, giving units if possible, and provide a good title for the graph, too.  Use the graphical results to describe the patterns if any that exist in the data as focused toward the research question. Bottom line is to explain the features of the graph that are relevant to conforming or departing to what is expected by your null hypothesis !-->
  
## Now with LESS data (more variables give less data due to NA's) create the approate graphic displays to answer the question.
  
```{r}
dd2 <- mydata %>% group_by(race,cappun) %>% summarize(count=n()) %>% mutate(prcnt=count/sum(count))


# the group_by followed by summarize(count=n())

basicC <- ggplot(dd2,aes(x=race,y=count,fill=cappun))
basicC + geom_bar(stat="identity",position="dodge")

#Now for percentage plot

basicCC <- ggplot(dd2,aes(x=race,y=prcnt*100,fill=cappun)) 
basicCC + geom_bar(stat="identity", position = "dodge")
```
   
The graphs are in the same format as in the master branch. The information is targeted better with a smaller scope of observations. In the first slide there is a smaller number of the white race at around 400 reports in favor of capital punishment. This number is still almost three times larger than the other two races. Both the black and other races have less than 100 reports in favor. The opposing reports are drastically different in the white race colun, with less than 100 reports in favor. The opposing reports are drastically different in the white race column, with just under half of all the favorable reports. 
   
### Numerical Descriptive Results

<!--Numerical results go here. Use the numerical results to describe the patterns if any that exist in the data as focused toward the research question!-->
  
```{r}
table2 <- xtabs(~race + cappun, data=mydata)
rowPerc(table2)
colPerc(table2)
```
  
In the numerical results, we see similar numbers to the master branches results. The white race with a 67% in favor of capital punishment report. The black race contains 47% in favor, and the other race with 61%. This mimics the numbers of the above chart, and can be seen as well in the master branch. 
    
## Inferential Results

<!--State hypothesis clearly.  Make sure your discussion of the inferential test covers all the aspects that the test output produces, such as test statistic, p-value etc.  Make a decision about the null hypothesis, explain the assumptions on which the selected test/procedure was based, and why the chosen procedure satisfys the assumptions and is appropriate to answer the research question!-->

```{r}
chisq.test(table2)
chisqtestGC(table2)
```

```{r}
source(file="fisherXY.R")
```

```{r}
fisherXY(mydata$race, mydata$cappun, confL= .01, printAll= "TRUE")
```
  
Using the CHI test and the Fisher test the numerical results gave us insight into the differences between each comparison using the new data. The CHI Test information provides a probability value of less than .000000000000000022 percent. This number is too small. 
The Fisher test confirms the comparisons of each race. The white race when compared to the black race was significantly different. The resulting p value was .00000174. The white and other race were compared and the p value that resulted was 0.2565. In the last section of comparison, the black and other races were compared and the resulting p value was 0.06299. This observation of the Fisher Test is a replication of the process used in the master branch. To evaluate this information, the hypothesis pertaining to the inclusion of race when determining favor of capital punishment, we are looking into both test's results. The p value of both the CHI squared, and the Fisher Test are not in favor of the results, and we now reject the null hypotheses. 
  
  
# Discussion and Conclusion

<!--Discussion and conclusion here.  If you found a relationship be sure to consider whether the relationship occurs because one of the variables causes the other, or whether they perhaps are related for some other reason.  Watch the chapter 6 videos from the George Town videos collection.!-->
 
   
Using the CHI test and the Fisher test the numerical results gave us insight into the differences between each comparison using the new data. The CHI Test information provides a probability value of less than .000000000000000022 percent. This number is too small. 
The Fisher test confirms the comparisons of each race. The white race when compared to the black race was significantly different. The resulting p value was .00000174. The white and other race were compared and the p value that resulted was 0.2565. In the last section of comparison, the black and other races were compared and the resulting p value was 0.06299. This observation of the Fisher Test is a replication of the process used in the master branch. To evaluate this information, the hypothesis pertaining to the inclusion of race when determining favor of capital punishment, we are looking into both test's results. The p value of both the CHI squared, and the Fisher Test are not in favor of the results, and we now reject the null hypotheses. 
  
